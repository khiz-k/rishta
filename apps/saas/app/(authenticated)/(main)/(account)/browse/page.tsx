...
// Warm gradient based on first letter
...
export default function BrowsePage() {
	...
	return (
		<div className="space-y-6">
			<PageHeader title="Discover" subtitle={`${profiles.length} profiles available`} />
						...
						return (
							<Card key={p.id} className="overflow-hidden group transition-all duration-300 hover:shadow-lg hover:border-primary/20">
								...
								<div className={`relative h-28 bg-gradient-to-br ${grad}`}>
									...
									{p.isVerified && (
										<Badge className="absolute top-3 left-3 bg-white/20 backdrop-blur-sm text-white text-xs border-0">
											✓ Verified
										</Badge>
									)}
								</div>
								...
								<CardContent className="pt-10 pb-4">
									<Link href={`/browse/${p.userId}`} className="block">
										<div className="flex items-start justify-between">
											...
											{p.createdBy !== "self" && (
												<Badge className="bg-amber-500/10 text-amber-500 text-xs shrink-0">
													👨‍👩‍👧 Family
												</Badge>
											)}
										</div>
										...
										<div className="mt-3 grid grid-cols-2 gap-2 text-sm text-muted-foreground">
											{p.location && (
												<span className="flex items-center gap-1.5">
													<MapPinIcon className="size-3.5 text-primary/60" />{p.location}
												</span>
											)}
											{p.profession && (
												<span className="flex items-center gap-1.5">
													<BriefcaseIcon className="size-3.5 text-primary/60" />{p.profession}
												</span>
											)}
											{p.education && (
												<span className="flex items-center gap-1.5">
													<GraduationCapIcon className="size-3.5 text-primary/60" />{p.education}
												</span>
											)}
											{p.height && (
												<span className="flex items-center gap-1.5">
													<RulerIcon className="size-3.5 text-primary/60" />{p.height} cm
												</span>
											)}
										</div>
										...
										<div className="flex flex-wrap gap-1.5 mt-3">
											...
											{p.maritalStatus !== "never_married" && <Badge className="bg-muted text-muted-foreground text-xs">{p.maritalStatus.replace(/_/g, " ")}</Badge>}
										</div>
									</Link>
