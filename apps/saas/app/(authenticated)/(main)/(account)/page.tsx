...
export default function DiscoverPage() {
	...
	// Competition stats for current profile
	const currentProfileUserId = profiles[currentIndex]?.userId;
	const { data: profileStats } = useQuery({
		...orpc.interests.profileStats.queryOptions({ input: { profileUserId: currentProfileUserId || "" } }),
		enabled: !!currentProfileUserId,
	});
				...
				>
					...
					>
						<Card className="w-full overflow-hidden relative select-none">
							...
							{/* Competition badge */}
							{profileStats && profileStats.totalInterested > 0 && (
								<div className="bg-amber-500/5 border-b border-amber-500/10 px-4 py-2 flex items-center justify-center gap-2 text-xs">
									<span className="text-amber-500 font-medium">🔥 {profileStats.totalInterested} {profileStats.totalInterested === 1 ? "person" : "people"} interested</span>
									{profileStats.topBid > 0 && <span className="text-muted-foreground">· Top bid: {profileStats.topBid} credits</span>}
								</div>
							)}
							...
							<CardContent className="pt-2 pb-6">
								...
								{showBidSlider && (
									...
									>
										...
										{profileStats && profileStats.topBid > 0 && (
											<p className="text-[10px] text-amber-500/70 mt-1 text-center">
												Top bid is {profileStats.topBid} credits — bid higher to stand out
											</p>
										)}
									</motion.div>
								)}
